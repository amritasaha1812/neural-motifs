...
def filter_dets(boxes, obj_dists, obj_scores, obj_classes, rel_inds, pred_scores):
    """
    Filters detections....
    :param boxes: [num_box, topk, 4] if bbox regression else [num_box, 4]
    :param obj_dists: [num_box, num_obj_classes]
    ...
    obj_dists_np = obj_dists.data.cpu().numpy()
    boxes_out = boxes.data.cpu().numpy()

    return boxes_out, objs_np, obj_scores_np, obj_dists_np, rels, pred_scores_sorted
